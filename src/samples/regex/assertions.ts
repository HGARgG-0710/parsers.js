 ...
 * inside a lookahead
 */
export const lookahead = (regexp: RegExp) =>
 ...
 * inside a lookbehind
 */
export const lookbehind = (regexp: RegExp) =>
	regex(`(?<=${regex.contents(regexp)})`)
 ...
 */
export const neg_lookbehind = (regexp: RegExp) =>
	regex(`(?<!${regex.contents(regexp)})`)
 ...
 */
export const begin = (regexp: RegExp) => regex(`^${regex.contents(regexp)}`)

/**
 * Creates a regular expression that ends with `regexp`
 */
export const end = (regexp: RegExp) => regex(`${regex.contents(regexp)}$`)
