...
const char_ranges = (...ranges: (string | [string, string])[]) =>
	ranges
		.map((r) => (typeof r === "string" ? r : `${r[0]}-${r[1]}`))
		.join("")
 ...
 */
export function charclass(...ranges: (string | [string, string])[]) {
	return regex(`[${char_ranges(...ranges)}]`)
}
...
export namespace charclass {
	 ...
	 * either a number, or be a string, written in hex
	 */
	export const uni_hex = (code: string | number) => () =>
		regex(`\\u{${code}}`)
