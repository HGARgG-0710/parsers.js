import type { IConcreteRegexFinalizer } from "../../interfaces.js"
...
function prepareValidState(frag: Fragment) {
	return frag.patch(new MatchState())
}

export class NFARegexFinalizer implements IConcreteRegexFinalizer {
	static readonly instance = new NFARegexFinalizer()
	...
	private toState(regex: Regex.Raw) {
		...
		return prepareValidState(frag).inState
	}

	toConcrete(regex: Regex.Raw) {
		return new NFARegexMatcher(this.toState(regex))
	}
