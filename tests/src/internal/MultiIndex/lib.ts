...
const levels = new MethodTest(
	"levels",
	function (this: MultiIndex, expected: number) {
...
const get = new MethodTest(
	"get",
	function (this: MultiIndex, sameAs: Iterable<number>) {
		basicGetTest(this, sameAs)
	}
)

const last = new MethodTest(
	"last",
	function (this: MultiIndex, expected: number) {
...
const sliceWhole = new MethodTest(
	"sliceWhole",
	function (this: MultiIndex, sameAs: Iterable<number>) {
...
// * `to === MissingArgument`
const sliceEnd = new MethodTest(
	"sliceEnd",
	function (this: MultiIndex, from: number, sameAs: Iterable<number>) {
		assert(array.same(this.slice(from), sameAs))
	}
)

// * `to !== MissingArgument, to < 0`
const sliceToNegative = new MethodTest(
	"sliceToNegative",
	function (
		this: MultiIndex,
		from: number,
...
// * `to !== MissingArgument, to >= 0`
const sliceToPositive = new MethodTest(
	"sliceToPositive",
	function (
		this: MultiIndex,
		from: number,
...
const prevLevel = new MethodTest(
	"prevLevel",
	function (this: MultiIndex, sameAs: Iterable<number>) {
		this.prevLevel()
		basicGetTest(this, sameAs)
...
const resizeLesser = new MethodTest(
	"resizeLesser",
	function (this: MultiIndex, toLength: number, sameAs: Iterable<number>) {
		assert(toLength < this.levels)
		basicResizeTest(this, toLength, sameAs)
	}
)

const resizeGreaterOrEqual = new MethodTest(
...
const extend = new MethodTest(
	"extend",
	function (this: MultiIndex, withSubIndex: number[], sameAs: number[]) {
		this.extend(withSubIndex)
		basicGetTest(this, sameAs)
	}
)

const from = new MethodTest(
	"from",
	function (this: MultiIndex, index: number[]) {
		this.from(index)
